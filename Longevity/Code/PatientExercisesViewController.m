...
//    [self setTimer];
    ...
    // array of screen cap images which may get replaced with video
    ExerciseVideos = [NSArray arrayWithObjects:
                          ...
                          @"sitStand",
                          @"tandemWalking",
                          @"reachingWhileStanding",
                          @"toeReach",
                          @"toeStand",
                          @"videoNotAvailable",
                      
                          @"seatedArmRaise",
                          @"seatedBicepCurlRight",
                          @"seatedBicepCurlResistance",
                          @"seatedLegExtend",
                          @"seatedLegExtendResistance",
                          @"videoNotAvailable",
                          @"videoNotAvailable",
...
//    // Timer
//    if(self.timer == nil) {
//        self.timer = [NSTimer scheduledTimerWithTimeInterval:10.0 target:self selector:@selector(goHome) userInfo:nil repeats:NO];
//    }
//}
...
- (void) goHome {
    ...
    // Load New View
    [self performSegueWithIdentifier:@"goHomeFromExercises" sender:self];
...
- (void)alertView: (UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    // Ok
    if (buttonIndex == 1) {
        [self.noteButton setImage:[UIImage imageNamed:@"addNoteActive.png"] forState:UIControlStateNormal];
    }
}
...
- (void)playVideo:(NSInteger)videoIndex {
    ...
    //Set Url
    NSURL *movieURL = [[NSBundle mainBundle] URLForResource:[ExerciseVideos objectAtIndex:videoIndex] withExtension:@"m4v"];
    ...
    moviePlayer.view.frame = CGRectMake(724, 98, 275, 155);
    ...
    [self performSelector:@selector(playBackStarted) withObject:nil afterDelay:1];
